...
# -*- coding: utf-8 -*-
## xmdict.py
## Octopus MDict Dictionary File (.mdx) and Resource File (.mdd) Exporter
##
...
def extract_mdd(fname, datafolder):
    glos = {}
    f = open(fname, 'rb')
    ...
    for current_record_block_size, decompressed_block_size in record_block_info_list:
        ...
        while i < len(key_block_list):
            key_start, key_text = key_block_list[i]
            if key_start - offset >= len(current_record_block_text):
                break
            if i < len(key_block_list)-1:
                key_end = key_block_list[i+1][0]
            else:
                key_end = len(current_record_block_text) + offset
            i += 1
            data = current_record_block_text[key_start-offset:key_end-offset]
            # write out
            output_fname = ''.join([datafolder, key_text.replace('\\', os.path.sep)]);
            if not os.path.exists(os.path.dirname(output_fname)):
                os.makedirs(os.path.dirname(output_fname))
            of = open(output_fname, 'wb')
            of.write(data)
            of.close()
        offset += len(current_record_block_text)

def extract_mdx(fname, encoding='', substyle=False):
...
if __name__ == '__main__':
    ...
    if ext == os.path.extsep + 'mdx':
        extract_mdx(args.filename, args.encoding, args.substyle)
    ...
    if (os.path.exists(mdd_filename)):
        ...
        extract_mdd(mdd_filename, args.datafolder)
